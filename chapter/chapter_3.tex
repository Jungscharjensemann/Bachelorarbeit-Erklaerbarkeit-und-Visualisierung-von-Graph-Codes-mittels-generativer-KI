...
%Da die Adjanzenzmatrix eines Graph Codes eine zweidimensionale, quadratische Matrix ist, wird im Allgemeinen zuerst die Matrix zeilenweise (s) und dann spaltenweise (t) durchlaufen und für den Eintrag des Merkmalspaares (s,t) in der Matrix der Wert in jedem Graph Code aus der Eingabe bestimmt und in der Matrix des Ergebnisses eingetragen.

%Die in \cref{sec3:model:par:mechanism-use-cases:alg:sim,sec3:model:par:mechanism-use-cases:alg:dif} beschriebenen Algorithmen würden, wie auch die Operationen \textit{Vereinigung} und \textit{Subtraktion} durch die Komponente \textit{GraphCodeCollection} bereitgestellt werden. mit in die Komponente \textit{GraphCodeCollection} aufgenommen werden.
...
%Im nächsten Abschnitt wird ein Konzept zur Transformierung der Metriken von Graph Codes beschrieben.
...
\begin{itemize}
    ...
    % Hier auf die Verarbeitung im Kontrollfluss eingehen...
...
%Anmerken dass Metrik assoziierten Information ist...
...
Erweiterung der bestehenden Metriken...
...
% Des Weiteren Erweiterung / Ergänzung zu Seq UC-1.3 (auch durch neue Informationen (voriger Abschnitt)).
% Erklärung zu Seq.
...
\input{chapter/chapter_3/tables/table-summary-mappings}
...
%\paragraph{Getroffene Entscheidungen}
